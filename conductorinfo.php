...
<div style="margin:30px;margin-right:30px;border:5px;border-radius:5px;z-index:2;overflow:hidden;background-color:#111; width: 95%; height:100%;margin-bottom: 30px; padding:10px;">
	...
	<div style="margin: 30px;background-color:grey;">
			<div style="background-color:#333;width:100%;height:30px;">
				<font style="font-size:150%; color:white;font-family: caviar;text-align: left;"> CONDUCTOR LOGIN</font>
			</div>
			<form oninput="validate()" onsubmit="return validate()" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
					<div style="padding-left:15%;width:90%;align-content: center">
						...
						<br><br>
						<p id="cardnumber" style="display:contents;"><font style="color:red">Invalid CONDUCTOR ID: Should greater than 5 digit</font></p>
						<br><br>
	...
	<font style="font-size:150%; color:white;font-family: caviar;text-align: left;">
		...
		INFORMATION:<br><br>
			...
			if($_POST)
			{
				$sql="select * from conductor where conductor_id=$num;";
				$res=$conn->query($sql);
				if($res->num_rows>0)
				{
					while($row=$res->fetch_assoc())
					{
						echo "CONDUCTOR ID -> ".$row["conductor_id"]. "<br>" . 
                             "NAME -> ".$row["name"]. "<br>" . 
                             "AGE -> ".$row["age"]. "<br>". 
                             "AADHAR -> ".$row["aadhar"]. "<br>" . 
                             "MOBILE -> ".$row["mobile"]."<br>".
                             "BUS ALLOTED->".$row["bus_no"];

					}
				}
				else
				{
					echo "CONDUCTOR DOES NOT EXISTS";
				}
			}
			?>
	</font></div></div>
